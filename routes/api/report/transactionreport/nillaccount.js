...
async function getCurrentBal(res, value, cId) {
  ...
  await PurchaseVoucher.find({
    ...
    .then(PurchaseVoucher => {
      ...
      currentBalance = openingBalance1 + credit - debit - openingBalance2;
...
// @type    get
//@route    /api/report/transactionreport/nillaccount/allnillbal
...
router.get(
  ...
  async (req, res) => {
    await AddLedger.find({
      ...
      allLedgerNillbal = AddLedgerResult;
    });
    await allLedgerNillbal.forEach((item, index) => {
      let data = {};
      let ans;
      ...
      getCurrentBal(res, item._id, req.user.companyId).then(() => {
        ...
        data.opBalance = indivisualOpeningBalance;
        data.currentBalance = currentBalance;
        console.log(data.ledgerName);
        if (data.currentBalance === 0) {
          fData.push(data);
        }
        if (index + 1 === allLedgerNillbal.length) {
          res.json(fData);
        }
      });
    });
